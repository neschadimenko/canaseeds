...
{
?>
<?/*?><div class="bx_item_title" style="border-bottom: 0px"><h1><span><?
	echo (
...
?>
		...
		<table style="width: 50%;">
			<tr>
			<td class="a-left">
			<span class="pduct-sku">Артикул <?echo($arResult['PROPERTIES']['CML2_ARTICLE']['VALUE']);?></span>
			</td>
			<td class="a-right">
				<?$APPLICATION->ShowViewContent('store_amount');?>
			</td>
			</tr>
		</table>
		</div>
...
?>
	...
	<div class="Oprice product-cart"><div class="price-box"><span class="regular-price"><span class="price" id="<? echo $arItemIDs['PRICE']; ?>"><? echo $arResult['MIN_PRICE']['PRINT_DISCOUNT_VALUE']; ?></span></span></div></div>
	<?$this->EndViewTarget();?>
...
<?
	...
	{
?>
	<ul class="block-attributes-product-view" style="width:100%;">
<?
		foreach ($arResult['DISPLAY_PROPERTIES'] as &$arOneProp)
		{// /images/properties/CML2_MANUFACTURER.png
?>			<?//dump($arOneProp["CODE"]);// Koval Roman?>
		<li><img src="/images/properties/<? echo $arOneProp['CODE']; ?>.png" alt="<? echo $arOneProp['NAME']; ?>" title="<? echo $arOneProp['NAME']; ?>"><span><?
			echo (
...
<div id=rightcol>
	...
	<?$APPLICATION->ShowViewContent('skuprop');?>
...
?>	
	...
	<div class="product-options">
		<dt><label class="required "><? echo htmlspecialcharsex($arProp['NAME']); ?><em>*</em></label></dt>
		<div class="bx_size_scroller_container"><div class="input-box" style="align: center;">
			
			<ul class="product-options options-list " id="<? echo $arItemIDs['PROP'].$arProp['ID']; ?>_list" style="width: <? echo $strWidth; ?>;">
<?
...
<i title="<? echo $arOneValue['NAME']; ?>"></i><span class="label" title="<? echo $arOneValue['NAME']; ?>"><label class="qty-options"><? echo $arOneValue['NAME']; ?></label></span></li>
<?
			}
?>
			</ul>
...
?>  <?$this->SetViewTarget('count');?>
	<span class="item_section_name_gray" style="color: #fff;"><? echo GetMessage('CATALOG_QUANTITY'); ?></span>
	<div class="item_buttons vam" style="align:center;">
	
		<span class="item_buttons_counter_block">
			<!--<a href="javascript:void(0)" class="bx_bt_button_type_2 bx_small bx_fwb" id="<?// echo $arItemIDs['QUANTITY_DOWN']; ?>">-</a>-->
			<input id="<? echo $arItemIDs['QUANTITY']; ?>" type="text" class="input-text qty" value="<? echo (isset($arResult['OFFERS']) && !empty($arResult['OFFERS'])
					? 1
...
<?
	...
	{
?>
			<button class="button product-cart btn-approve" onclick="javascript:void(0);" id="<? echo $arItemIDs['BUY_LINK']; ?>"><a href="javascript:void(0);" class="buybuttonnn" ><?// echo $buyBtnMessage; ?></a><span class="buybuttonnn"><? echo $buyBtnMessage; ?></span></button>
<?
	}
	if ($showAddBtn)
	{
		...
		<div class="item_info_section">
			...
			?>
				<div class="bx_item_description">
					<div class="short-description product-view" ><h2 style="color: #276008;"><? echo GetMessage('FULL_DESCRIPTION'); ?></h2></div>
			<?
				...
				{
					?><p style="color: #276008;"><? echo $arResult['DETAIL_TEXT']; ?></p><?
				}
			?>
				</div>
			<?
...
?>
   ...
   #leftcol {
    ...
    width: 195px; /* Ширина левой колонки */
	left: 80%;
    background: none repeat scroll 0 0 #8dab66; /* Цвет фона содержимого */
    padding: 10px; /* Поля вокруг текста */
	z-index: 999;
   }
   #rightcol { /* Этот слой накладывается поверх */ 
    position: relative; /* Относительное позиционирование */
    left: 350px; /* Положение от левого края */
    top: -300px; /* Положение от верхнего края */
    width: 195px; /* Ширина правой колонки */
	align:center;
	text-align: center;
    background: none repeat scroll 0 0 #8dab66; /* Цвет фона */
